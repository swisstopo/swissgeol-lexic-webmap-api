...
type ResponseContent<TResponse> = TResponse extends {
...
type ResponseContentBody<TResponse> = ResponseContent<TResponse> extends never
  ? never
  : ResponseContent<TResponse>[keyof ResponseContent<TResponse> & string];
...
export type OpenApiResponseValidationBody<
  TPath extends OpenApiPath,
  TMethod extends OpenApiMethod,
  TStatusCode extends OpenApiStatusCode<TPath, TMethod>,
> = ResponseContentBody<OperationResponses<TPath, TMethod>[TStatusCode]>;

export type OpenApiResponseContentType<
  TPath extends OpenApiPath,
  TMethod extends OpenApiMethod,
  TStatusCode extends OpenApiStatusCode<TPath, TMethod>,
> = keyof ResponseContent<OperationResponses<TPath, TMethod>[TStatusCode]> &
  string;
...
> = {
  [TStatusCode in OpenApiStatusCode<TPath, TMethod>]:
    ...
    | {
        statusCode: TStatusCode;
        body: OpenApiResponseValidationBody<TPath, TMethod, TStatusCode>;
        payload: Buffer;
